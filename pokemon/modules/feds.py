...
from pokemon import dispatcher, OWNER_ID, SUDO_USERS, DEV_USERS, WHITELIST_USERS, LOGGER, MESSAGE_DUMP
from pokemon.modules.helper_funcs.handlers import CMD_STARTERS
from pokemon.modules.helper_funcs.misc import is_module_loaded, send_to_list
from pokemon.modules.helper_funcs.chat_status import is_user_admin
from pokemon.modules.helper_funcs.extraction import extract_user, extract_unt_fedban, extract_user_fban
from pokemon.modules.helper_funcs.string_handling import markdown_parser
from pokemon.modules.disable import DisableAbleCommandHandler

import pokemon.modules.sql.feds_sql as sql

from pokemon.modules.connection import connected
from pokemon.modules.helper_funcs.alternate import send_message
    ...
    if args:
        if args[0] == 'json':
            ...
            with BytesIO(str.encode(backups)) as output:
                output.name = "pokemon_fbanned_users.json"
                update.effective_message.reply_document(document=output, filename="pokemon_fbanned_users.json",
            ...
            for users in getfban:
                ...
                backups += "\n"
            with BytesIO(str.encode(backups)) as output:
                output.name = "pokemon_fbanned_users.csv"
                update.effective_message.reply_document(document=output, filename="pokemon_fbanned_users.csv",
